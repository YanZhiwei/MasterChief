...
namespace MasterChief.DotNet.ProjectTemplate.WebApi.Helper
{
    ...
    public sealed class SignatureHelper
    {
        ...
        public static string Create(string appSecret, string timestamp, string nonce)
        {
            ...
            Array.Sort(data);
            var signatureString = string.Join("", data);
        ...
        internal static CheckResult Validate(string signature, string timestamp, string nonce, string appSecret,
            int signatureExpiredMinutes)
        {
            string[] data = {appSecret, timestamp, nonce};
            Array.Sort(data);
            var signatureString = string.Join("", data);
            ...
            if (signature.CompareIgnoreCase(signatureString) && CheckHelper.IsNumber(timestamp))
            {
                ...
                if (minutes > signatureExpiredMinutes) return CheckResult.Fail("签名时间戳失效");
            }
