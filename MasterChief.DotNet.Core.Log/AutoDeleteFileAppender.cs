namespace MasterChief.DotNet.Core.Log
{
    ...
    public class AutoDeleteFileAppender : RollingFileAppender
    {
        ...
        private static readonly Type _declaringType = typeof(AutoDeleteFileAppender);
        ...
        protected void RemoveOldLogFiles()
        {
            try
            {
                string[] logFiles = Directory.GetFiles(_baseDirectory, "*.log", SearchOption.AllDirectories);
                List<FileInfo> fiLogList = new List<FileInfo>();
                foreach (string file in logFiles)
                {
                    fiLogList.Add(new FileInfo(file));
                }

                foreach (FileInfo item in fiLogList)
                {
