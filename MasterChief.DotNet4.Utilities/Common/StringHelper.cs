...
namespace MasterChief.DotNet4.Utilities.Common
{
    ...
    public static class StringHelper
    {
        ...
        public static string BuilderDelimiter(this string data, char delimiter)
        {
            ...
            {
                builder.AppendFormat("{0}{1}", c, delimiter);
            }
        ...
        {
            return long.Parse(data, NumberStyles.AllowThousands | NumberStyles.AllowDecimalPoint | NumberStyles.AllowLeadingSign);
        }
        ...
        public static string UnEscape(this string data)
        {
            if (!string.IsNullOrEmpty(data))
            {
                ...
                while (index != count)
                {
                    if (Uri.IsHexEncoding(data, index))
                    {
                        builder.Append(Uri.HexUnescape(data, ref index));
                    }
                    else
                    {
        ...
        public static string WrapText(this string data, int maxWidth)
        {
            ...
            if (maxWidth > 0 && count > maxWidth)
            {
                ...
                int wrapIndex = builder.Length / maxWidth;

                for (int i = 0; i < wrapIndex; i++)
                {
                    int position = i * maxWidth;
        ...
        /// <returns>长度</returns>
        public static int CaluLength(string data)
        {
            if (string.IsNullOrEmpty(data))
            {
                return 0;
